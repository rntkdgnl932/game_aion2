...
def game_screen(cla):
    ...
    try:
        ...
        full_path = "c:\\my_games\\game_aion2\\data_basic\\imgs\\check\\out\\screen_out_1.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        imgs_ = imgs_set_(0, 30, 1920, 1040, "one", img, 0.8)
        if imgs_ is not None and imgs_ != False:
            print("screen_out_1", imgs_)
            click_pos_2(970, 530, cla)

        full_path = "c:\\my_games\\game_aion2\\data_basic\\imgs\\check\\out\\character_bool_1.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        imgs_ = imgs_set_(0, 30, 1920, 1040, "one", img, 0.8)
        if imgs_ is not None and imgs_ != False:
            print("character_bool_1", imgs_)
            click_pos_reg(imgs_.x, imgs_.y, cla)
        full_path = "c:\\my_games\\game_aion2\\data_basic\\imgs\\check\\out\\server_in.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        imgs_ = imgs_set_(0, 30, 1920, 1040, "one", img, 0.8)
        if imgs_ is not None and imgs_ != False:
            print("server_in", imgs_)
            click_pos_reg(imgs_.x, imgs_.y, cla)
        full_path = "c:\\my_games\\game_aion2\\data_basic\\imgs\\check\\out\\game_start_btn.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        imgs_ = imgs_set_(0, 30, 1920, 1040, "one", img, 0.8)
        if imgs_ is not None and imgs_ != False:
